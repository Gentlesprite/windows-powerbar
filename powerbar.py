...
class PowerMenu(QMenu):
    def __init__(self):
        ...
        if self.loadConfig(only_check=True):  # 生成配置文件
            self.pid, self.notice, self.dtype, self.start_with_windows = os.getpid(), None, None, None
    ...
    def loadConfig(self, only_check=False):
        # config = self.config.getConfig(exist_ok=True)  # 加载配置
        # print(config)
        if isinstance(self.config_content_dict, dict):  # 配置必须为字典
            if only_check:
                ...
                try:
                    ...
                    self.config.updateConfig(config_dict=self.config_content_dict, notice=self.notice,
                                             dtype=self.dtype,
    ...
    def quit(self):
        pid, notice, dtype, startWithWindows = self.matchCurrentConfigFile()
    ...
    def taskBarMenu(self):  # 定义任务栏右边的图标
        ...
        # 获取电源配置
        power_cfg = PowerMenu.getPowerConfig()  # 重新获取电源配置
