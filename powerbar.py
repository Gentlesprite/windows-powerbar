...
class PowerMenu(QMenu):
            ...
            if only_check:
                ...
                try:
                    if self.config_content_dict['notice'] == True and self.config_content_dict['type'] in range(
                        ...
                        #         logger.success(f'{self.config_content_dict["type"]}:成功')
                        #     else:
                        #         logger.error(f'self.config_content_dict的值为{self.config_content_dict["type"]},i.value的值为{i.value}')
                        logger.error(k0aduiotype_v1address)
                        for k0aduiotype, v1address in k0aduiotype_v1address.items():  # 对提示音字典进行遍历 key:type的
                            if k0aduiotype.value == self.config_content_dict['type']:  # 如果提示音字典中的键等于配置文件中的值
                                logger.error('1111111111111111111111111111111')
                    ...
                    # if self.config_content_dict['type'] == AudioType.Default.value:
                    ...
                    # 匹配电源计划
                    # 获取电脑的所有的电源配置
                    power_cfg: dict = PowerMenu.getPowerConfig()  # 获取所以电源选项 k：name v：guid
    ...
    def taskBarMenu(self):  # 定义任务栏右边的图标
        ...
        logger.debug(self.power_box)
        # 将菜单设置为系统托盘图标的上下文菜单

        voice_cfg = {}
        for i in AudioType:
            voice_cfg[i] = i.value
        logger.debug(voice_cfg)
        ...
        logger.debug(self.voice_box)

        # for k0v1_0,k0v1_1 in zip(power_cfg.items(),voice_cfg.items()):
        #     self.action = QAction(k0v1_0[0], self, checkable=True, data=(k0v1_0[0], k0v1_0[1], self.action))  # 创建QAction对象，文本为电源选项名称
        #     self.voice_buttons = QAction(k0v1_1[0], self, checkable=True, data=(k0v1_1[0], k0v1_1[1], self.voice_buttons))
        #     self.power_box[k0v1_0[0]] = self.action  # 存储电源选项的字典 k0类型(别名)v1控件地址
        #     self.voice_box[k0v1_1[0]] = self.voice_buttons
        #     self.menu.addAction(self.action)  # 将QAction对象添加到菜单中
        #     self.voice_menu.addAction(self.voice_buttons)
    ...
    def updateVoiceBoxState(self, data):
        if data == self.is_notice_button.data():
            ...
            logger.debug(f'voice:{voice},action:{action},地址{address}')
    ...
    # def updateVoiceBoxState(self, data):
    #     voice, action, address = data
    #     logger.debug(f'voice:{voice},action:{action}')
    #     notice_name = ''
    ...
    #         self.is_notice_button.setMenu(self.voice_menu)
    #     logger.debug(f'提示音调整为:{notice_name} 编号:{self.dtype}')
    #     self.config.updateConfig(config_dict=self.config_content_dict, dtype=self.dtype, notice=self.notice)
