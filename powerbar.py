...
# todo 0.9方向：解决当软件移动时开机目标自动改变的功能
...
class PowerMenu(QMenu):
    ...
    def quit(self):
        _pid, notice, dtype, startWithWindows, current_power_plan = self.matchCurrentConfigFile()
        pid = 0
        self.config.updateConfig(config_dict=self.config_content_dict, pid=0, notice=notice, dtype=dtype,
    ...
    def taskBarMenu(self):  # 定义任务栏右边的图标
        ...
        # self.quit_button.setIcon(QIcon(':powercfg.ico'))
...
class SaveUserConfig:
    ...
    without_ext_software_name = os.path.splitext(PowerMenu.software_name)[0]
